...
class FallStateMachine:
    ...
    def step(self, frame: dict) -> State:
        ...
        if any(v is None or (isinstance(v, float) and v != v) for v in (theta, omega, v_y, rho, hip_height)):
            return self.state  # 個別特徵仍是 NaN(缺失率未達凍結門檻但關鍵值缺),本幀不判斷
        ...
        if self.state == State.NORMAL:
            ...
            if self._lying_criteria(theta, rho, hip_height):
                ...
                self._transition(t, State.NORMAL, "1.0s 內未達躺姿,逾時退回")

        elif self.state in (State.ON_GROUND, State.CONFIRMED, State.ALERTED):
            self._step_lying_or_recovery(t, theta, rho, hip_height)

        return self.state

    def _step_lying_or_recovery(self, t: float, theta: float, rho: float, hip_height: float) -> None:
        cfg = self.config
        ...
        if self._recovery_criteria(theta, hip_height):
            ...
            if t - self._recovery_start_t >= cfg.recovery_hold_s:
