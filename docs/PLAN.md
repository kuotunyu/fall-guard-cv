...
| D33 | 2026-07-22 | **README 拿掉「為什麼做這個專案」動機段，`supersedes` D-無(第一次寫入時即由 CLAUDE.md 規則帶出的預設項目，這是使用者本人明確決定的例外)**：CLAUDE.md 公開文案守則明文要求 README 開頭需有第一人稱動機段，Phase 4 也照規則擬了草稿(聚焦練習關鍵點特徵工程)並讓使用者過目調整過方向；使用者最終決定這個專案不需要這段，直接拿掉整個章節，不只是拿掉草稿標記。這是使用者對自己專案內容的直接決定，予以尊重並執行；CLAUDE.md 本身(跨多個專案共用的範本)不因單一專案的個別決定而修改，僅在本專案的 Decision Log 記錄此例外。連帶修正：第 10 章 checklist 第 2 項移除；`tests/test_docs.py` 的 `REQUIRED_README_SECTIONS` 移除「為什麼做這個專案」；同時趁這次全文複查，修正模型選型章節「表1/表2/表3」原本編號跳號且順序錯亂(表1→表3→表2)的問題，改成 pose(表1)→分類器(表2)→VLM(表3) 依序排列，並把一處未加超連結的 `PLAN.md §7.2` 文字引用補上連結 | `uv run pytest -q` 43 passed（含更新後的 `test_readme_has_required_sections`）；`uv run python scripts/check_public_text.py README.md docs/PLAN.md tests/test_docs.py` 全綠 |
...
| D47 | 2026-07-24 | **重新考慮 D33，動機段落加回 README（supersedes D33）**：D33 當初是使用者看過 AI 起草的動機段草稿後決定整段拿掉。本輪多維度收尾複查（32-agent 審查）獨立發現：README 開頭仍掛著過時的「🚧 開發中」提示（與 Phase 0-7 已完工的事實矛盾），且完全沒有 CLAUDE.md 要求的第一人稱動機段——兩者疊加使讀者看到的第一屏內容既不準確也缺乏定位。`models/xgboost/README.md`（HF 模型卡）第 14 行其實已有一句可用的動機句「回應高齡長輩獨居時常見的照護擔憂」，只是沒搬進主 README。故重新起草一版動機段（換句話說、非照搬模型卡原句）取代「開發中」提示，兩件事一併處理。**此為 AI 草稿，比照 CLAUDE.md「我會提供或請你依專案主題擬一段自然的個人動機供我修改」的要求，需使用者親自審閱修改後才算定稿**；若使用者仍偏好 D33 的原決定（不放動機段），可直接改回 | 收尾複查發現的 README.md:8「開發中」標籤過時問題與 README.md:1「無動機段」問題性質相關（同一段落位置），一併處理比分兩次修改更乾淨；modification 已套用於 README.md，待使用者審閱 |
| D48 | 2026-07-24 | **修正 `window_ground_truth()` 對 Le2i 標籤語意的誤判**：此函式「只含 0 ⇒ 剔除」的規則是照 URFD 官方 -1/0/1 三值標籤設計(0=躺姿判定的模糊過渡帶)，但 `prepare_le2i.py` 產出的 Le2i 標籤只有 0/1 兩值(0=明確的跌倒區間外)，套用同一規則會把 Le2i fall 影片裡真正的負例視窗誤判成「歧義」整批剔除。目前這個 bug 從未反映在任何已發布報表——`write_cross_report()` 只印事件級指標(不受此函式影響)，`build_xgb_stat_samples()` 目前也完全沒被套用在 Le2i 資料上——是收尾複查(32-agent 審查)找到的一個潛伏但尚未爆炸的邏輯缺陷。修正：`VideoData` 加 `has_ambiguous_label` 旗標(`load_all_videos()` 依 `processed_dir` 是否為 `LE2I_PROCESSED_DIR` 自動判斷)，`window_ground_truth()` 只在該旗標為真(URFD)時才把「只含 0」視為歧義剔除，Le2i 直接算負例。同時修正 `write_cross_report()`/`write_report()` 補上 FP/小時分母定義、樣本數透明度、`confirm_seconds` 搜尋網格的巢狀 CV 局限揭露(收尾複查另外幾項 finding)。**驗證**：重跑 `--protocol loso`/`cross`、`--model xgb --protocol loso` 三份報告，`git diff` 確認除了本次刻意新增的文字說明外，所有數字(P1-P5 各項指標、Le2i Sensitivity=0.559/Specificity=0.000/FP=110.475)完全沒有變化——證實這個 bug 確實從未被目前的評估流程觸發過，修正不影響任何既有已發布數字 | `uv run python scripts/evaluate.py --model rule --protocol loso/cross`、`--model xgb --protocol loso`；`git diff docs/results/*.md` 三份報告皆確認除新增文字外數字零變化；`uv run pytest -q` 60 passed |
| D49 | 2026-07-24 | **修正 FSM 恢復計時在特徵缺失(遮擋)期間會被靜默撤銷跌倒告警的安全性 bug**：`_step_lying_or_recovery()` 的 `_recovery_start_t`(恢復站立計時起點)只在 `feats_valid=True` 時才會因不滿足恢復條件而重置，`feats_valid=False`(遮擋)分支完全沒有對應的逾時重置——只要曾經有 1 幀「疑似站起」的有效證據，之後不管遮擋多久，只要牆鐘時間累積滿 `recovery_hold_s`(預設 2.0s)就會判定「已恢復直立」，把已 CONFIRMED/ALERTED 的跌倒狀態撤銷回 NORMAL、清空 `_last_alert_t`——等於在沒有持續視覺證據下靜默撤銷告警。這是收尾複查(32-agent 審查)發現、並用最小重現腳本驗證屬實的真實邏輯漏洞：軀幹可見(不觸發 `_is_frozen` 凍結)但腳踝被家具遮擋(hip_height 缺失)是常見的居家場景，恰好落在「凍結機制管不到、但恢復計時仍在跑」的縫隙。對比同一函式裡 `_lying_accum_start_t` 早已有 `lying_jitter_tolerance_s` 抖動容忍重置機制，兩者處理方式不對稱，且安全方向搞反——`FALLING` 逾時退回 NORMAL 寧可漏判也不卡死是合理的保守設計，但套在「已確認要撤銷告警」的恢復判定上方向應該相反(寧可誤判為未恢復)。修正：新增 `_last_recovery_true_t` 追蹤最後一次滿足恢復條件的時間、新增獨立的 `FSMConfig.recovery_jitter_tolerance_s`(預設 0.5s，獨立於 `lying_jitter_tolerance_s` 以便未來各自調整)，`feats_valid=False` 分支仿照躺姿累積計時的既有模式，超過容忍上限就把 `_recovery_start_t` 重置為 None。**驗證**：重跑 URFD LOSO 五折、Le2i cross-dataset 評估，`git diff` 確認所有已發布數字零變化(此 bug 從未被 URFD/Le2i 現有影片觸發過)，證實修正是純粹的前瞻性安全加固、不影響任何既有結果 | 同 D48 驗證證據；另外在 scratchpad 寫過一次獨立重現腳本(僅驗證用，未寫入 repo)，針對修正後的程式碼實測：ALERTED 狀態下餵 1 幀有效恢復證據 + 之後連續 3 秒遮擋幀(torso 可見、hip_height=NaN)，修正後 state 仍維持 ALERTED(未被靜默撤銷)；修正前會變回 NORMAL 這點沿用收尾複查本身的獨立重現結果，未重複驗證 |
| D50 | 2026-07-24 | **修正批次姿態抽取 `persist=True` 跨影片污染 ByteTrack 追蹤器狀態**：`pose.py::extract_video_pose()` 對每支影片都傳 `persist=True`，但 `prepare_data.py`/`prepare_le2i.py` 在整批 70/130 支影片的迴圈裡重複用同一個 `model` 物件呼叫它——`persist=True` 是 ultralytics 官方針對「單一連續串流」設計的參數，`on_predict_start()` 遇到 `persist=True` 會直接跳過重建 tracker，導致 `BYTETracker.frame_id` 計數跨影片不歸零，理論上會讓每支影片(除了批次裡第一支)第一次出現的人在該幀因 `STrack.is_activated` 只在 `frame_id==1` 才立即為真而被濾掉。收尾複查(32-agent 審查)發現此問題，判定「未讀取 track_id 的下游程式碼故不影響現有結果」。**實測驗證(比對讀 ultralytics 原始碼更進一步)**：用兩組真實影片對(adl-01→adl-02、fall-01→fall-02)比較「同一 model 物件對第二支影片」在 persist=True(舊行為)vs persist=False(修正後)下的完整輸出——conf/xyn/bbox 逐幀值 100% 相同(max diff=0.0)、偵測/未偵測的幀序列(NaN pattern)完全一致，只有 `track_id` 這個從未被任何下游邏輯讀取的欄位不同。證實理論上的疑慮「is_activated 過濾會讓該幀整個被丟棄」並未真的發生(ultralytics 對未啟用的 track 似乎仍保留原始偵測框，只是不賦予有效 ID)，此 bug 對既有 npz 資料無影響，**不需要重新抽取**。修正：`extract_video_pose()` 加 `persist: bool = False` 參數(批次抽取預設 False，讓 ultralytics 在每支新影片開始時正確重建 tracker)，`prepare_data.py`/`prepare_le2i.py` 呼叫端不需改動(沿用新預設值)；`detect.py` 的即時單一串流路徑(`PoseEstimator.infer()`)維持 `persist=True` 不變 | scratchpad 兩份驗證腳本(僅驗證用，未寫入 repo)：`verify_pose_fix.py`(adl-01/adl-02)、`verify_pose_fix2.py`(fall-01/fall-02)，皆確認 conf/xyn 逐幀零差異；`uv run pytest -q` 60 passed |
| D51 | 2026-07-24 | **肉眼獨立驗證 Le2i 3 段 adl 影片的標籤正確性，並找到跨資料集誤報的更直接根因**：收尾複查(32-agent 審查)建議用逐幀 activity code 欄位交叉驗證 `parse_annotation()` 的「有無表頭」反推規則；實測發現此欄位語意不明(官方 README 未定義)、且 127 段 fall 檔案裡有 78 段(61%)找不到跌倒區間專屬 code、3 段 adl 檔案裡有 2 段本身含有 fall 範例影片的區間專屬 code,不採用此欄位。改用更直接的方式：用 cv2 對 3 段 adl 影片(le2i-coffee01-26/le2i-coffee02-50/le2i-coffee02-52)各抽 6 張取樣幀肉眼核對，**確認 3 段全部是同一種佈景**——受試者刻意在地上鋪一張床墊、緩慢走近後躺上去,是近似 URFD「躺床」ADL 的居家版本,不是真的跌倒,adl 標籤正確無誤。這個發現同時強化(而非取代)D46 的 `confirm_seconds` 根因假說：這 3 段全部觸發跨資料集誤報,不是因為標籤錯誤,而是因為「緩慢受控地躺上床墊」的姿態終態跟真跌倒很像,套用極短的評估用 `confirm_seconds=0.3s` 時難以區分——跟 URFD 自己「躺床」ADL 靠 `v_y` 全程不超過觸發閾值就能正確分辨(README「誤報案例分析」一節)形成有趣對照:同樣是「緩慢躺下」，Le2i 這 3 段仍會觸發誤報，代表問題出在確認秒數這個時間維度的把關被跳過，而不是下墜速度這個特徵本身失效 | scratchpad 影格截圖(僅驗證用，未寫入 repo)：`extract_adl_frames.py` 對 3 段影片各抽 6 張取樣幀,肉眼確認床墊佈景與緩慢躺下動作 |
| D52 | 2026-07-24 | **揭露 `TUNE_CONFIRM_SECONDS_GRID`(D16 引入)的巢狀 CV 局限**：收尾複查(32-agent 審查)發現 D16 新增的 `confirm_seconds` 折內調參候選範圍(0.3–1.5s)，是根據「URFD 全部 30 段 fall 影片」(涵蓋每折未來的 test 影片)的探索性分析定案，而非只看各折 train 資料——`tune_fsm_timing()` 選最終值時確實只用 train_ids(這部分乾淨)，但候選邊界本身已隱含全資料集資訊，屬於超參數搜尋空間設計層級(而非最終取值)的資訊洩漏，不是嚴格巢狀 CV。佐證：P1-P5 五折折內調參後全部選中同一組邊界值(1.5s/0.3s，見 `docs/results/rule_baseline.md`)，顯示這個邊界對結果有實質影響，可能讓 Sensitivity 有輕微樂觀偏誤。已在 `scripts/evaluate.py` 的 `write_report()` 增補此局限說明，隨 `rule_baseline.md` 每次重新產出自動附上，不需手動維護。評估：URFD 僅 5 位受試者、30 段 fall 影片，再切一塊獨立探索子集會讓本已很小的資料集更捉襟見肘，性價比低，此局限選擇誠實揭露而非重新設計切分 | `scripts/evaluate.py::write_report()` 新增局限說明段落；重跑 `--protocol loso` 並 `git diff docs/results/rule_baseline.md` 確認除新增文字外所有數字零變化(同 D48/D49 驗證方式) |
...
- [x] 權重回 `models/`；**驗收**：`uv run python scripts/evaluate.py --model xgb --protocol loso` 本機重現 Colab 數字（±0.01）→ **完成。過程中發現並修正一個真的重現不了的 bug（D18：兩處視窗排除條件邏輯飄移），修正後全部 15 項指標誤差皆為 0.000，完全重現**
...
1. 標題 + badges（tests / Python 3.11 / uv / MIT）
2. 為什麼做這個專案（第一人稱動機段，D33 曾拿掉、D47 收尾複查後重新加回，AI 草稿待使用者審閱）+ 完工狀態陳述 + **demo GIF**
...
- [x] README 第一人稱動機段 → D33 曾決定拿掉，D47 收尾複查後重新考慮加回（AI 草稿，待使用者審閱定稿）
...
- [x] demo.gif ≤8MB 且 README 首屏可見 → 6,385,421 bytes（約 6.09 MiB／6.39 MB，D26-D32 多次重錄後的最終版；先前記錄的 2.43MB 是 D25 稽核當下的舊版本，已於本次收尾複查更新），README.md 首屏可見，`tests/test_docs.py` 守門
