...
| s | 63.0 | — |
| **m** | **68.8** | **正式評估與部署**:準確度與延遲的平衡點,4090 上仍可輕鬆跑到下方「即時偵測」一節的 FPS 目標 |
| l | 70.4 | — |
| x | 71.6 | — |
...
| 角色 | 模型設定 | 用途 |
|---|---|---|
| 主力 | `GEMINI_MODEL`<br/>.env 設定,預設 `gemini-3.5-flash-lite` | 跌倒確認後,描述現場姿態/環境/嚴重程度,寫進 Discord 通報文字 |
| 備援/評審 | `OPENAI_MODEL`<br/>.env 設定,預設 `gpt-5-mini` | 本專案主線未呼叫,保留供日後人工比較兩家描述品質用 |
